...
class GaussianCopulaSynthesizer(BaseSynthesizer):
    ...
    def sample_with_conditions(
        ...
        GaussianCopula has native support for conditional sampling via SDV's
        sample_from_conditions method. This is more efficient than reject sampling.
        ...
        logger.info(f"Generating {num_rows} conditional rows with Gaussian Copula")
        start_time = time.time()
        ...
        if sdv_conditions:
            # Try to merge conditions for efficiency
            merged = processor.merge_sdv_conditions(target_rows, sdv_conditions)

            if merged:
                # All conditions are EQ, use merged condition
                ...
                # Have IN conditions, build multiple SDV conditions
            ...
            # No SDV-native conditions, just sample
            synthetic_df = self._synthesizer.sample(target_rows)
